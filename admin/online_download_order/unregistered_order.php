...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div>
                <h5 id="no_pay">خریدهای پرداخت نشده</h5>
                <span id="got_to_pay">نمایش لیست خریدهای پرداخت شده</span>
            </div>
            <table>
                <tr>
                    <th>ردیف</th>
                ...
                <?php
                    $argument =
                        [
                            'fields' => [],
                            'values' => [],
                            'query' => 'SELECT *, `user_order_links`.`date`, `user_order_links`.`user_full_name` FROM `payline` LEFT JOIN `user_order_links` ON `payline`.`user_id` = `user_order_links`.`user_id`  WHERE (`user_order_links`.`status` = 0) ORDER BY `payline`.`id` DESC;'
                        ];
                        ...
                        if (count($result) === 0)
                        {
                            echo '
                                <script type="text/javascript" language="JavaScript">
                                    alert("هیچ کاربری وجود ندارد!");
                                    window.location.href = baseURL + "manage_online_download.php";
                                </script>';
                        }
                        else
                        {
                            for ($i = 0; $i < count($result); $i++)
                            {
                                echo '
                                    <tr>
                                        <td>'.($i + 1).'</td>
                                        <td>'.getUserName($result[$i]['user_id']).'</td>
...
<script type="text/javascript" language="JavaScript">
    ...
    var got_to_pay = get._id('got_to_pay');
    for (var i = 0; i < delete_user.length; i++)
    {
     ...
     got_to_pay.addEventListener('click', function ()
     {
         window.location.href = baseURL + "manage_online_download.php";
     });
